...
export default {
    ...
    execute: async (message) => {
        ...
        if (faction.ramo !== RAMO) {
            return message.reply(`🔫 Desmontar roubado é coisa da **Contrabando de Armas**! Sua fac é de **${FACTIONS[faction.ramo].name}**.`);
        }

        if (await hasActiveCooldown(userId, 'ferido')) {
            return message.reply('🤕 Tá todo moído! Nem mola de suspensão consegue puxar. Procura um **Médico** (`k tratar @medico`).');
        }
        ...
        if (!isDev) {
            const cd = await prisma.cooldown.findUnique({
                where: { userId_command: { userId, command: 'desmanchar' } }
            });
            if (cd && cd.expiresAt > new Date()) {
                const minutos = Math.ceil((cd.expiresAt - new Date()) / 60000);
                return message.reply(`🔧 A oficina ainda tá terminando de moer o carro da última leva! Volta em **${minutos} minutos**.`);
            }
            const nextTime = new Date(Date.now() + cooldownMinutes * 60 * 1000);
            await prisma.cooldown.upsert({
                where: { userId_command: { userId, command: 'desmanchar' } },
                update: { expiresAt: nextTime },
                create: { userId, command: 'desmanchar', expiresAt: nextTime }
            });
        }
        ...
        // 💥 FALHA: a polícia chegou na oficina
        if (chance < risco) {
            ...
            if (Math.random() < 0.15) {
                ...
                feridoMsg = '\n🤕 **Na fuga, o macaco hidráulico escapou na sua canela.** Tá ferido por 15 minutos!';
            }

            return message.reply(`🚨 **A ROTA INVADIU A OFICINA!**\nVocê teve que largar tudo e pular o muro dos fundos. Prejuízo de **$${multa.toLocaleString('pt-BR')}** em peças perdidas.${feridoMsg}`);
        }

        // ✅ SUCESSO: carro virado em peça e grana
